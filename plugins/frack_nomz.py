...
USER_ADD = re.compile('^!nomz \+([0-9]+)(?: (maandag|dinsdag|woensdag|donderdag|vrijdag|zaterdag|zondag|vandaag))?$')
USER_REMOVE = re.compile('^!nomz -([0-9]+)(?: (maandag|dinsdag|woensdag|donderdag|vrijdag|zaterdag|zondag|vandaag))?$')
...
class Plugin(plugins.Base):
  ...
  def Process(self, server, message):
    ...
    elif message.content == '!nomz nieuwedag':
      self.loadToday(server)
    elif message.content == '!nomz wie':
      wiestring = ''       
      totaal = 0
    ...
    elif message.content == '!nomz planning':
      for day in days:
        wiestring = ''       
        totaal = 0
        for user in self.schedule[day]:
